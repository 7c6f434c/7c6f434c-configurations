...
for i in "snd-hda-intel power_save=5 index=1,0" "af_packet" "acpi-cpufreq"     \
	 "cpufreq-ondemand" "coretemp" "battery" "xhci-hcd" "uvcvideo"         \
	 "snd-pcm" "kvm-intel" "aesni-intel" "crc32c-intel" "intel-powerclamp" \
	 "asus-nb-wmi" "usb-storage" "mousedev" "evdev" "psmouse" "msr" "tun"  \
	 "ac" "battery" "thermal" "fuse" "loop" "rtc-cmos" "sysdig-probe"      \
	 "ax88179_178a" "smsc75xx" "asix" "cdc_ether" "usbhid" "hid-generic"   \
	 "xhci-pci"  "af-packet" "wmi" "ctr" "arc4" "ath9k nohwcrypt=1" "fuse" \
	 "ccm" "nbd"                                                           \
         ;
  do      
	  echo $i
...
while ! su nobody -s /bin/sh -c "/var/setuid-wrappers/su -c true"; do echo Please enter root password to continue; done
