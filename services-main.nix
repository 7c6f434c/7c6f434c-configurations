{pkgs, config, ...}:
{
    ...
    cron = {
      systemCronJobs = [
#	in local time
	"34 4 * * * root ( [ -e /dev/sdb5 ] && ! [ -d /media/sdb5 ] && pmount /dev/sdb5 && ([ -f /media/sdb5/backup/auto-backup-here ] && /root/script/backup_notebook /media/sdb5/backup ; ) ; sync && pumount /dev/sdb5 ) &> /var/log/sdb5-backup.log "
	"29 4 * * * root ( [ -e /dev/sdb1 ] && ! [ -d /media/sdb1 ] && pmount /dev/sdb1 && ([ -f /media/sdb1/backup/auto-backup-here ] && /root/script/backup_notebook /media/sdb1/backup ; ) ; sync && pumount /dev/sdb1 ) &> /var/log/sdb1-backup.log "
	"44 4 * * * root ( [ -e /dev/sdc5 ] && ! [ -d /media/sdc5 ] && pmount /dev/sdc5 && ([ -f /media/sdc5/backup/auto-backup-here ] && /root/script/backup_notebook /media/sdc5/backup ; ) ; sync && pumount /dev/sdc5 ) &> /var/log/sdc5-backup.log "
	"39 4 * * * root ( [ -e /dev/sdc1 ] && ! [ -d /media/sdc1 ] && pmount /dev/sdc1 && ([ -f /media/sdc1/backup/auto-backup-here ] && /root/script/backup_notebook /media/sdc1/backup ; ) ; sync && pumount /dev/sdc1 ) &> /var/log/sdc1-backup.log "
