...
rec {
  ...
  allEudevUdevRules = ["${eudev}/var/lib/udev/rules.d/*.rules"];

  makeUdevInitrd = { extraToolset ? [], extraToolsetWithTargets ? []
    , extraToolsetCommands ? "", extraContents ? []
  ...
  qemuLauncherFun = makeOverridable ({initrd}: 
    writeScript "qemu-launcher" ''
      qemu="$1"
      shift
      "$qemu" -kernel ${initrd.kernel}/bzImage \
  ...
  setuidWrapper = runCommand "setuid-wrapper" {} ''
    ...
    ${gcc}/bin/gcc ${<nixpkgs> + "/nixos/modules/security/setuid-wrapper.c" } \
      -Wall -O2 -DWRAPPER_DIR='"/var/current-system/setuid/wrappers"' \
  ...
  nixosServiceScript = name: config:
  ...
  nixosEtcSelect = nixos: filename:
    ...
    in (builtins.getAttr (builtins.head (builtins.attrNames selected)) selected).source;

  initrdContentEntry = symlinkPrefix: pkg: {
    object = pkg;
    symlink = symlinkPrefix + "/" + pkg.name;
  };

  shebang = "#!${stdenv.shell}\n";
}
