...
pkgs.lib.makeExtensible (self: with self; {
  ...
  openjml-binary-set = pkgs.runCommandNoCC "openjml" {} ''
    ...
    ( grep -rl '/lib64/ld-linux-x86-64[.]so[.]2' "$out/openjml" ) | xargs -l1 "${pkgs.lib.getExe pkgs.patchelf}" --set-interpreter "${pkgs.glibc}/lib64/ld-linux-x86-64.so.2" --set-rpath "${pkgs.lib.makeLibraryPath [pkgs.zlib "${placeholder "out"}/openjml/jdk"]}"
    (  find "$out/openjml" -name '*.so' -type f ) | xargs -l1 "${pkgs.lib.getExe pkgs.patchelf}" --set-rpath "${pkgs.lib.makeLibraryPath [pkgs.zlib "${placeholder "out"}/openjml/jdk"]}"
  '';
})
